...
CREATE TABLE tenants (
  ...
  contact_phone text,
  plan_id subscription_plan NOT NULL DEFAULT 'starter',
...
CREATE UNIQUE INDEX one_active_control_plan_per_product
  ON control_plans(tenant_id, product_id) WHERE is_active;
...
-- Defense in depth: API transactions set app.tenant_id before accessing tenant data.
...
CREATE POLICY tenant_products ON products USING (tenant_id = current_setting('app.tenant_id', true)::uuid);
CREATE POLICY tenant_control_plans ON control_plans USING (tenant_id = current_setting('app.tenant_id', true)::uuid);
CREATE POLICY tenant_inspection_logs ON inspection_logs USING (tenant_id = current_setting('app.tenant_id', true)::uuid);
CREATE POLICY tenant_audit_logs ON audit_logs USING (tenant_id = current_setting('app.tenant_id', true)::uuid);
