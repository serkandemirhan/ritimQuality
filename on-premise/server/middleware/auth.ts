...
export async function requireAuth(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  try {
    ...
    const result = await withDatabase((client, tenantId) => {
      if (claims.tenantId !== tenantId) throw new Error('Kurum kimliği geçersiz.');
      return client.query('SELECT role,status FROM users WHERE id=$1 AND tenant_id=$2', [claims.userId, tenantId]);
    });
