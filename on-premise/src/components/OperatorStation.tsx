...
export const OperatorStation: React.FC<OperatorStationProps> = ({
  ...
  const draftKey = 'quality:draft:' + OnPremApi.scope();
  ...
  const handleEvidenceUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const attachment=await OnPremApi.uploadEvidence({fileName:file.name,mimeType:file.type||'application/octet-stream',kind,dataUrl});
