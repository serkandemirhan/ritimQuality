...
export function PendingInspections({onSaved}: {onSaved: () => void}) {
  const [items,setItems]=useState(OnPremApi.pendingInspections);
  const [busy,setBusy]=useState(false);
  useEffect(()=>{ const refresh=()=>setItems(OnPremApi.pendingInspections()); window.addEventListener('quality-pending-changed',refresh);return()=>window.removeEventListener('quality-pending-changed',refresh);},[]);
