...
export const ProductManagement: React.FC<ProductManagementProps> = ({
  ...
  const handleDrawingUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const reader=new FileReader(); reader.onload=()=>setDrawingUrl(String(reader.result||'')); reader.readAsDataURL(file);
  };
          ...
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="grid lg:grid-cols-[minmax(320px,42%)_1fr]">
              <div className="flex min-h-72 items-center justify-center border-b border-slate-200 bg-slate-950 p-5 lg:border-b-0 lg:border-r">
                <img src={selectedProduct.defaultDrawingUrl || SHAFT_BUSHING_SVG} alt={selectedProduct.name} className="max-h-[360px] max-w-full object-contain" referrerPolicy="no-referrer" />
              </div>
              <div className="p-6">
            ...
            {filteredProducts.map(product => {
              ...
              return (
                <button key={product.id} type="button" onClick={() => setSelectedProductId(product.id)} className="grid w-full grid-cols-[44px_minmax(0,1fr)_auto] items-center gap-3 border-b border-slate-100 px-4 py-3 text-left transition last:border-b-0 hover:bg-blue-50/50 sm:grid-cols-[52px_minmax(180px,1.4fr)_minmax(130px,1fr)_minmax(120px,.8fr)_100px_24px]">
                  <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-lg bg-slate-900 sm:h-11 sm:w-11"><img src={product.defaultDrawingUrl || SHAFT_BUSHING_SVG} alt="" className="h-full w-full object-contain" /></div>
