...
export function AnnotationCanvas({imageUrl,pointNo,annotations,onChange}:{imageUrl:string;pointNo:number;annotations:ImageAnnotation[];onChange:(items:ImageAnnotation[])=>void}) {
  ...
  useEffect(()=>{setFirst(null);setSelected('');setHistory([]);setFuture([]);},[imageUrl,pointNo]);
  ...
  const applyColor=(nextColor:string)=>{setColor(nextColor);if(!selected)return;commit(annotations.map(item=>item.id===selected?{...item,color:nextColor}:item));};
  ...
  const buttons:[Tool,string][]=[['select','Seç/Taşı'],['pin','Pin'],['measurement_line','Ölçüm Çizgisi'],['area','Alan'],['note','Not'],['delete','Sil']];
  ...
  return <section className="overflow-hidden rounded-2xl bg-slate-950">
    <div className="flex flex-wrap items-center gap-1 border-b border-slate-700 bg-slate-900 p-2">
      ...
      <span className="mx-1 h-6 border-l border-slate-700"/>
      <span className="mr-1 text-[11px] text-slate-400">Renk</span>
      {COLORS.map(value=><button key={value} type="button" title={value} aria-label={`Anotasyon rengi ${value}`} onClick={()=>applyColor(value)} style={{backgroundColor:value}} className={`h-5 w-5 rounded-full border-2 ${color===value?'border-white':'border-slate-600'}`}/>)}
      <input aria-label="Özel anotasyon rengi" title="Özel renk" type="color" value={color} onChange={event=>applyColor(event.target.value)} className="h-6 w-7 cursor-pointer border-0 bg-transparent p-0"/>
      <span className="mx-1 h-6 border-l border-slate-700"/>
    ...
    <div className="overflow-auto p-3">
      <div ref={stage} onClick={click} style={{transform:`scale(${zoom})`,transformOrigin:'center'}} className="relative mx-auto min-h-72 max-w-4xl cursor-crosshair bg-slate-900">
        {isPdfMedia(imageUrl)?<object data={url} type="application/pdf" className="pointer-events-none h-[60vh] w-full bg-white"/>:<img src={url} alt="Anotasyon yapılan teknik görsel" className="pointer-events-none max-h-[65vh] w-full object-contain"/>}
          ...
          {annotations.map(item=>{
            ...
            if(item.type==='measurement_line'){
              ...
              return <g key={item.id} data-annotation-id={item.id} onClick={event=>choose(item.id,event)} onPointerDown={event=>beginDrag(item.id,event)} onPointerMove={event=>dragItem(item.id,event)} onPointerUp={endDrag} className="cursor-move">
                ...
                {isSelected&&<><circle aria-label="Başlangıç noktasını taşı" cx={item.startX} cy={item.startY} r="2.2" fill="#fff" stroke={itemColor} strokeWidth="1" vectorEffect="non-scaling-stroke" className="cursor-grab" onPointerDown={event=>beginDrag(item.id,event,'start')} onPointerMove={event=>dragItem(item.id,event)} onPointerUp={endDrag}/><circle aria-label="Bitiş noktasını taşı" cx={item.endX} cy={item.endY} r="2.2" fill="#fff" stroke={itemColor} strokeWidth="1" vectorEffect="non-scaling-stroke" className="cursor-grab" onPointerDown={event=>beginDrag(item.id,event,'end')} onPointerMove={event=>dragItem(item.id,event)} onPointerUp={endDrag}/></>}
              </g>;
            }
