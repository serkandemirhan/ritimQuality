...
export function PushSettings(){
  ...
  const enable=async()=>{
    setBusy(true);try{
      if(!('serviceWorker' in navigator)||!('PushManager' in window)||!('Notification' in window))throw new Error('Bu tarayıcı cihaz bildirimlerini desteklemiyor. Zil simgesini kullanabilirsiniz. iPhone’da uygulamayı Ana Ekrana ekleyip oradan açın.');
      ...
      const config=await SaasApi.pushConfig() as {enabled:boolean;publicKey:string};if(!config.enabled)throw new Error('Cihaz bildirim hizmeti sunucuda henüz etkinleştirilmemiş. Uygulama içi bildirimler aktif.');
