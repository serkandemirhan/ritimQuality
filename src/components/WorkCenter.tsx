...
export function WorkCenter({users,products,currentUser,onInspect}:{users:User[];products:Product[];currentUser:User;onInspect:(productId:string)=>void}) {
  ...
  const reload=async()=>setData(await SaasApi.work() as Work);
  ...
  return <div className="space-y-4">
    ...
    {tab==='cases'&&<div className="space-y-3">
      ...
      {selected&&<form key={selected.id+':'+selected.version} className="space-y-3 rounded-xl border bg-white p-5" onSubmit={event=>{event.preventDefault();const fields=new FormData(event.currentTarget);void run(async()=>{const updated=await SaasApi.updateCase(selected.id,{version:selected.version,ownerId:fields.get('ownerId'),dueAt:new Date(String(fields.get('dueAt'))).toISOString(),state:fields.get('state'),containment:fields.get('containment'),rootCause:fields.get('rootCause'),action:fields.get('action'),verification:fields.get('verification')});setSelected(updated as Case);});}}>
    ...
    {tab==='tasks'&&<div className="space-y-3">
      {manager&&<form className="grid gap-3 rounded-xl border bg-white p-4 sm:grid-cols-2" onSubmit={event=>{event.preventDefault();const form=event.currentTarget;const fields=new FormData(form);void run(async()=>{await SaasApi.createTask({title:fields.get('title'),ownerId:fields.get('ownerId'),productId:fields.get('productId')||undefined,dueAt:new Date(String(fields.get('dueAt'))).toISOString()});form.reset();});}}>
