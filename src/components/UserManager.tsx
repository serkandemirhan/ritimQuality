...
import { CheckCircle2, ChevronDown, Edit3, Eye, HardHat, LogIn, Search, Shield, Sparkles, Trash2, UserPlus, Users, X } from 'lucide-react';
...
const ROLE_INFO: Record<UserRole, { label: string; description: string; className: string; icon: React.ReactNode }> = {
  admin: {
    label: 'Yönetici',
    description: 'Kullanıcılar, firma ayarları ve tüm kalite kayıtlarını yönetir.',
  ...
  operator: {
    label: 'Operatör',
    description: 'Atandığı istasyonda ölçüm girişi yapar ve sonuçları görüntüler.',
    className: 'border-emerald-200 bg-emerald-50 text-emerald-700',
    icon: <HardHat className="h-4 w-4" />,
  },
  auditor: {
    label: 'Denetçi',
    description: 'Kalite kayıtlarını, raporları ve izlenebilirlik verilerini inceler.',
    ...
    <div className="space-y-5">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-black text-slate-900">Kullanıcı yönetimi</h2>
              <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-600">{users.length}</span>
            </div>
            <p className="mt-1 text-sm text-slate-500">Kullanıcıları, rollerini ve hesap durumlarını tek ekrandan yönetin.</p>
          </div>
          <button type="button" onClick={openCreate} className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-blue-700">
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="grid gap-3 border-b border-slate-200 p-4 md:grid-cols-[minmax(260px,1fr)_200px_170px]">
          <label className="relative">
            ...
            <input type="search" value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="İsim, e-posta veya bölüm ara" className="h-10 w-full rounded-xl border border-slate-300 bg-white pl-9 pr-3 text-sm text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
          </label>
          <select value={roleFilter} onChange={(event) => setRoleFilter(event.target.value as 'all' | UserRole)} className="h-10 rounded-xl border border-slate-300 bg-white px-3 text-sm text-slate-700 outline-none focus:border-blue-500" aria-label="Role göre filtrele">
            <option value="all">Tüm roller</option>
            {(Object.keys(ROLE_INFO) as UserRole[]).map((role) => <option key={role} value={role}>{ROLE_INFO[role].label}</option>)}
          </select>
          <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as 'all' | User['status'])} className="h-10 rounded-xl border border-slate-300 bg-white px-3 text-sm text-slate-700 outline-none focus:border-blue-500" aria-label="Duruma göre filtrele">
            <option value="all">Tüm durumlar</option><option value="active">Aktif</option><option value="suspended">Pasif</option>
          </select>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[850px] text-left">
            <thead className="bg-slate-50 text-xs font-bold uppercase tracking-wide text-slate-500">
              <tr><th className="px-5 py-3">Kullanıcı</th><th className="px-4 py-3">Rol</th><th className="px-4 py-3">Bölüm / İstasyon</th><th className="px-4 py-3">Durum</th><th className="px-5 py-3 text-right">İşlemler</th></tr>
            </thead>
                ...
                return (
                  <tr key={user.id} className="text-sm text-slate-700 transition hover:bg-slate-50/80">
                    <td className="px-5 py-4"><div className="flex items-center gap-3">
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-slate-100 font-black text-slate-600">{user.name.slice(0, 2).toLocaleUpperCase('tr-TR')}</div>
                      <div className="min-w-0"><div className="flex items-center gap-2 font-bold text-slate-900"><span className="truncate">{user.name}</span>{isCurrent && <span className="rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-bold text-blue-700">Siz</span>}</div><div className="truncate text-xs text-slate-500">{user.email}</div></div>
                    </div></td>
                    ...
                    <td className="px-5 py-4"><div className="flex items-center justify-end gap-1">
                      {!isCurrent && user.status === 'active' && <button type="button" onClick={() => onUserSelect(user.id)} className="rounded-lg p-2 text-slate-500 hover:bg-blue-50 hover:text-blue-700" title="Bu kullanıcıya geç"><LogIn className="h-4 w-4" /></button>}
                      <button type="button" onClick={() => openEdit(user)} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-900" title="Düzenle"><Edit3 className="h-4 w-4" /></button>
                      <button type="button" onClick={() => deleteUser(user)} disabled={isCurrent || users.length <= 1} className="rounded-lg p-2 text-slate-500 hover:bg-red-50 hover:text-red-600 disabled:cursor-not-allowed disabled:opacity-30" title={isCurrent ? 'Aktif kullanıcı silinemez' : 'Sil'}><Trash2 className="h-4 w-4" /></button>
                    </div></td>
                  </tr>
                );
              })}
      ...
      <details className="group rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="grid gap-3 border-t border-slate-100 p-5 sm:grid-cols-2 lg:grid-cols-4">
          {(Object.keys(ROLE_INFO) as UserRole[]).map((roleKey) => { const role = ROLE_INFO[roleKey]; return <div key={roleKey} className="rounded-xl border border-slate-200 p-4"><div className={`inline-flex items-center gap-2 rounded-lg border px-2.5 py-1 text-xs font-bold ${role.className}`}>{role.icon}{role.label}</div><p className="mt-3 text-xs leading-5 text-slate-500">{role.description}</p></div>; })}
        </div>
      </details>
